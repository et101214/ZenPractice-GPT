...
document.getElementById("woodfish").onclick=()=>{woodfishOn=!woodfishOn;settings.woodfish=woodfishOn;store.set("settings",settings);const toggle=document.querySelector('[data-setting="woodfish"]');toggle.classList.toggle("on",woodfishOn);toggle.setAttribute("aria-pressed",woodfishOn);document.getElementById("woodfish").textContent="木魚："+(woodfishOn?"開":"關")};
...
renderLibrary();
document.querySelectorAll(".toggle").forEach(t=>{const k=t.dataset.setting,on=settings[k]!==false;t.classList.toggle("on",on);t.setAttribute("aria-pressed",on);t.onclick=()=>{settings[k]=!(settings[k]!==false);t.classList.toggle("on",settings[k]);t.setAttribute("aria-pressed",settings[k]);if(k==="woodfish"){woodfishOn=settings[k];document.getElementById("woodfish").textContent="木魚："+(woodfishOn?"開":"關")}store.set("settings",settings);msg("設定已更新")}});
